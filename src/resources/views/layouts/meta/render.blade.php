...
<meta @if ($meta->content) content="{{ route("image-filter", ["template" => "medium", "filename" => $meta->content]) }}" @endif @if ($meta->property) property="{{ $meta->property }}"  @elseif ($meta->name) name="{{ $meta->name }}" @endif>
@else
